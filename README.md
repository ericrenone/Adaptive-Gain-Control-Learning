...
### 3. Rational Inattention (RI)
Following Sims (2003), DPFAE optimizes a policy balancing utility against information-processing costs. The **Gain Adaptation Path** dynamically regulates sensitivity, analogous to the optimal Boltzmann distribution in RI models, effectively "ignoring" high-frequency noise that carries no structural information.
...
#### 🔑 Core Identity
- **Reactive Path (Fast Updates)**: Responds immediately to incoming errors:
  $$\theta_{t+1}^{(1)} = \theta_t^{(1)} - \eta \cdot \text{grad}_t$$
- **Adaptive Path (Gain-Controlled)**: Modulates magnitude via dynamic gain $\alpha$, suppressing variance:
...
| :--- | :--- | :--- | :--- | :--- |
| **Convergence** | Linear | Sublinear | Task-dependent | **Geometric (Ergodic)** |
